...
> 本文档不再更新：22年早期内容，个人认为 uniapp开发较简单，且《官方文档》易读易用实时
...
const handleUpload = () => {
  uni.chooseImage({
    ...
    success: async (res) => {
      ...
      // #ifdef APP
      file = { path: tempFilePaths[0], file: tempFilePaths[0] }
      if (tempFilePaths[0].indexOf('file://') === -1) file = await saveImageToPhone(tempFilePaths[0])
      const fileData = getFileDate(file?.path || file?.file)
      fileName = fileData.fileName
      fileType = fileData.fileType
      // #endif

      // 文件类型校验
      if (props?.type && props.type.indexOf(fileTypeList[fileType]) === -1)
        return uni.showToast({ title: `文件格式不正确，支持【${props.type}】`, icon: 'none' })
      const upInfo = {
        bucket: '1', // 默认私有, 1:私有 2:公开
        source: '4',
        storageType: '2', //阿里云
        ...props.upInfo,
        fileName,
      ...
      const reqUpFile = await uploadFile(upInfo, file, fileType)
      modelValue.value = reqUpFile.fileId
    }
  })
}
...
```js
import { FileApi, ossRequest } from '@/common/api'
...
/**
 * 文件上传方式1:[推荐]
 * @description 加签url，直传
 * @param file 文件
 */
export async function uploadFile(upInfo, file, fileType, callBack) {
  const reqUrl = await FileApi.UploadUrl({ data: upInfo })
  if (reqUrl.code === 200) {
    const { fileId, ossUploadUrl } = reqUrl.data
    let upFileOss

    // #ifdef H5
    upFileOss = await fetch(ossUploadUrl, {
      method: 'PUT',
      body: file,
      headers: { 'Content-Type': fileType }
    })
    if (upFileOss.ok) {
      const cBack = await FileApi.clientUploadCallBack(fileId)
      cBack.code === 200 && uni.showToast({ title: `上传成功`, icon: 'none' })
      callBack && callBack()
      return reqUrl.data
    }
    // #endif
    ...
    // #ifdef APP
    const fileArrayBuffer = await getFileArrayBuffer(file)
    upFileOss = await ossRequest(ossUploadUrl, fileArrayBuffer, fileType)
    if (upFileOss.statusCode === 200) {
      const cBack = await FileApi.clientUploadCallBack(fileId)
      cBack.code === 200 && uni.showToast({ title: `上传成功`, icon: 'none' })
      callBack && callBack()
      return reqUrl.data
...
/**
 * @description oss,根据文件id 获取文件url
 * @param fileId 文件ID
 */
...
/**
 * @description 根据文件url，获取文件type - 为app端提供
 * @param fileUrl 文件url
 */
export const getFileDate = (fileUrl) => {
  var fileName = fileUrl.split('/').pop() // 获取文件名部分
  var fileId = fileName.split('.').pop() // 获取文件类型（扩展名）

  for (var key in fileTypeList) {
    // eslint-disable-next-line no-prototype-builtins
    if (fileTypeList.hasOwnProperty(key) && fileTypeList[key] === fileId) {
      return { fileType: key, fileName }
    }
  }
}

/* 文件地址-转arraybuffer - 为app端提供 */
...
/**
 * @description 保存图片到本地应用私有目录 - 为app端提供
 * @param fileUrl 文件地址-临时/永久 文件路径，不支持网络图片路径
 * @return {path} 保存后文件的绝对路径，原文件路径将失效，不能再使用
 */
export function saveImageToPhone(fileUrl) {
  return new Promise((resolve, reject) => {
    uni.saveFile({
      tempFilePath: fileUrl,
      success: function (data) {
        return resolve({ path: plus.io.convertLocalFileSystemURL(data.savedFilePath) })
      },
      fail: function (err) {
        return reject(err)
      }
...
```js
export const ossRequest = (url, file, fileType) => {
  return new Promise((resolve, reject) =>
    uni.request({
      dataType: 'json',
      // #ifndef MP-WEIXIN
      ...
      method: 'PUT',
      loading: true,
      noToken: true, // 不需要token,个别接口不需要携带token
      header: {
...
> https://blog.csdn.net/qq_40745143/article/details/128955853

- 文件保存到本地的方法，使用 uni.saveFile 
  - 返回的是相对路径，直接使用可能存在问题，需转化为绝对路径
  - 默认即为应用存储沙盒
- 方案2：使用`plus.io`
  - https://www.html5plus.org/doc/zh_cn/io.html#
  - 注意：[安卓 分区存储概念](https://ask.dcloud.net.cn/article/id-36199__page-2#reply)
