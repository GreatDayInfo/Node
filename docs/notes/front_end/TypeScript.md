...
- 基础类型：`boolean number string symbol null undefined`
...
// void 没有类型，仅undefined可以作为void类型的值
// void定义的空值不能赋值给其他变量
let v:void = undefined
let srt:string = '123'  // 若 let srt:string = v 编译会报错！！！
...
TODO：https://www.bilibili.com/video/BV1r9YYzcEr4?spm_id_from=333.788.player.switch&vd_source=49059bedc59884104ea6ef0a6e552378&p=26

```
 interface类型,支持基于多个类型进行扩展-共同继承
...
> TS函数重载区别于其他语言中的重载，TS中是针对不同参数的个数和类型，推断返回值类型；一些其他强语言中，函数重载是指定义几个函数名相同，但参数个数或类型不同的函数，再调用时传入不同的参数，编译器会自动调用合适的函数。
...
function fn1(parmas:string,params2:any):void
// 执行函数逻辑的定义（应满足上述所有规则要求）,执行时会从上述规则中选择执行，具体根据使用时的传参来定
function fn1(parmas:any,parmas2?:any):void{
...
TODO：https://www.bilibili.com/video/BV1r9YYzcEr4?spm_id_from=333.788.videopod.episodes&vd_source=49059bedc59884104ea6ef0a6e552378&p=27
...
TODO：https://www.bilibili.com/video/BV1r9YYzcEr4?spm_id_from=333.788.videopod.episodes&vd_source=49059bedc59884104ea6ef0a6e552378&p=28
...
TODO：https://www.bilibili.com/video/BV1r9YYzcEr4?spm_id_from=333.788.videopod.episodes&vd_source=49059bedc59884104ea6ef0a6e552378&p=29
