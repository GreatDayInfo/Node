...
> Restful API 一种API约定规则，需要设计序言、全局(错误码、请求Base、Proxy等)参数、修改记录以及按功能划分的接口描述
...
> - 推荐文章 https://juejin.cn/post/7274893714971344953

- 怎么做？
  - 模拟一些容易在联调时出问题的接口，关键的接口即可，没必要100% mock，除非你想完全摆脱后端
  - 当后端接口已经稳定后，或者发生改动时
    - 此时删除mock接口，使用真实的接口更好，否则就要同步修改mock数据
  - 当收益大于成本时，可前端猜测/根据需求 自行mock后交给后端调整
- 
