...
- 单线程
  - 主线程为单线程，阻塞部分由线程池处理。
  - 不需要处理多线程的复杂性（如 线程同步、死锁等）
    - 不适合处理CPU密集型任务，CPU密集型会阻塞事件循环
  - 大多数的I/O请求都为异步请求，从而避免上下文频繁切换的耗时操作，具有高并发的特点
- 轻量级和高性能
  - 借助谷歌V8引擎，更高效的处理并发、异步等性能问题
  - 核心模块相对精简，大量功能借助三方模块实现
- 跨平台和生态丰富
- 异步队列
...
- 适用情况
  - 实时应用：在线游戏、聊天、实时通知等，适合处理高并发、大量数据流动
  - 单页应用：在线文档、社交应用，提供快速数据更新和高效的页面渲染
  - 说明
    - 对于CPU密集型任务的方案
      - 使用 Worker Threads 实现
      - 使用子线程
      - 使用C++ 插件
    - 对于大型复杂系统
      - 使用微服务架构：拆分为多个小的、独立的服务，可单独开发、部署和扩展
      - 使用TS
      - 使用良好的开发实践：模块化、测试驱动开发TDD、持续集成CI、持续部署CD 
...
packages:
 -	'main'
 -  'web/**'
# catalog 工作空间的功能，可将依赖项版本定义为可复用常量。
# 目录中定义后可在 package.json 文件中引用"react": "catalog:",
catalog:
  react: ^18.3.1
  redux: ^5.0.1
...
> - 自动化更新工具 [npm-check-updates](https://github.com/raineorshine/npm-check-updates)
> - 自动化更新工具能做的，pnpm和npm其实也都能！
> - [参考引用文档](https://ksh7.com/posts/npm-batch-update/index.html#%E6%9B%B4%E6%96%B0%E5%85%B6%E4%BB%96-packageManager-%E5%AE%89%E8%A3%85%E7%9A%84%E5%85%A8%E5%B1%80%E4%BE%9D%E8%B5%96)

- 最佳实践
  - **定期检查更新**：每月至少检查一次依赖更新
  - **测试后再部署**：更新后务必运行测试
  - **使用锁文件**：确保团队使用相同版本
  - **考虑自动化**：设置 CI/CD 流程自动检查依赖更新
  - **关注安全更新**：优先处理安全相关的更新
- 考虑使用以下工具自动化依赖更新：
...
# 更新所有依赖【依据semver 规范升级版本】
npm update
pnpm update

# 更新所有依赖【无视规范，使用最新版本】
npm update --latest
pnpm up --latest # 更新所有依赖到最新版本

# 更新全局范围的依赖
npm ls -g # 查看当前拥有哪些全局依赖
npm update -g
pnpm update -g

# 自动更新工具 npm-check-updates 使用
npm install -g npm-check-updates # 全局安装 ncu
ncu # 检查最新版本（无视当前的 semver 规范）

ncu -i --format group # -i 开启交互模式，空格 选中需升级的依赖，回车 升级；--format group 按 semver规范分组显示

ncu --target semver # 遵循package.json 中的 semver规范
ncu --target minor # 安装最新的 次版本和修订版
ncu --target patch # 只更新 修订版

ncu -g # 检查 npm 的全局依赖
ncu -g -p pnpm # 更新通过 pnpm 安装的依赖

npx npm-check-updates # 检查更新
