...
- 在 `v-for` 中使用模板引用：对应的 ref 中包含的值是一个数组，它将在元素被挂载后包含对应整个列表的所有元素
  - v-for 循环引用中，ref 数组**并不**保证与源数组相同的顺序
...
- 作用
  ...
  2. inheritAttrs 穿透配置【推荐设置为false】
     - 根标签存在多个，那么透传将失效
     - 默认开启，可手动配置禁用传值穿透
     - 【推荐】 设置为false避免样式穿透后的滥用，通过`v-bind="$attrs"`规范传递目标
       - **防止属性污染**：避免包装组件的根元素接收不需要的属性
       - **精确控制**：手动决定哪些属性传递给哪个子组件
       - **组件透明化**：让高阶组件表现得像原始组件一样
       - **避免冲突**：防止样式类或事件监听器的重复绑定
...
> `v-bind` 是 Vue.js 中的一个强大指令，允许开发者将数据绑定到 HTML 元素上，包括样式。通过使用 `v-bind`，你可以动态地根据组件的状态添加或修改样式，从而提升网页的样式灵活性
